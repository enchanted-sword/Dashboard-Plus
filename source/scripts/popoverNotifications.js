...
const buttonSelector = `[href="/blog/${primaryBlog.name}/activity"]`;
const smListSelector = '[data-css~="navigationLinks"]';
const customClass = 'ch-utils-popoverNotifications';
const app = document.getElementById('tumblr');
  ...
  return [
    ...
    notification.grouped ? {
      ...
      notification.hasTags ? {
        ...
        children: [{
          children: notification.sharePost.tags.map(tag => {
            return {
              href: `/rc/tagged/${tag}`,
...
const newNotificationPage = (date, notifications) => {
  return {
...
const onNotificationButtonClick = async function (event) {
  ...
  if (state !== 'open' && button.scrollHeight) {
    ...
    popover.querySelector('.loader').replaceWith(noact({
      ...
      children: Object.keys(notifications).sort((a, b) => Number(b) - Number(a)).map(date => newNotificationPage(date, notifications[date])) // otherwise it will always be backwards because of how entries are ordered
    }));
  } else {
